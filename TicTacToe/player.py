...
class RandomComputerPlayer(Player): #use inheritance to create a RandomComputerPlayer and a Human computer player
    def __init__(self, letter):
...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self,game):
        if len(game.available_moves()) == 9:
            square = random.choice(game.avaialble_mopves()) #randomly choose one
        else:
    ...
    def minimax(self, state, player):
        ...
        other_player = '0' if player == 'X' else 'X' #the other player .. so whatever letter is NOT

        # first we wan to check if the previous move is a winner
