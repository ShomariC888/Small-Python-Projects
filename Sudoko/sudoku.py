...
def is_valid(puzzle, guess, row, col):
    ...
    # and then the square
    #  but we want to get where the 3x3 square starts
    ...
    for r in range(row_start, row_start+3):
        for c in range(col_start, col_start+3):
            if puzzle[r][c] == guess: #if guess is in the 3x3 matrix
                return False
...
def solve_sudoku(puzzle):
    ...
    # step 1.1: if there is nowhere left, we're done because we only allowed valid inputs
    if row is None:
        return True
    
    # step2: if there is a place to put a number, then make a guess between 1 and 9
    for guess in range(1, 10): #range(1,10) is 1,2,3,....,9
        # step3: check if this is a valid guess
    ...
    # step 6: if none of the nmumbers we try work, then this puzzle is unsolvable
    return False
